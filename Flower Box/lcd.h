...
void drawDiagnostics(OLEDDisplay *display, OLEDDisplayUiState* state, int16_t x, int16_t y) {
	...
	if(wifiState == WiFiConnected)
		sprintf(number_buffer, "Wifi OK");
	else 
		sprintf(number_buffer, "Wifi OFF: %d", wifiState);

	display->drawString(0 + x, 0 + y, number_buffer);

	sprintf(number_buffer, "Wifi connect count %d", wifi_connect_count);
	display->drawString(0 + x, 12 + y, number_buffer);

	if (mqttState == ConnectedToMQTTServer)
		sprintf(number_buffer, "MQTT OK");
	else
		sprintf(number_buffer, "MQTT OFF: %d", mqttState);

	display->drawString(0 + x, 24 + y, number_buffer);

	sprintf(number_buffer, "MQTT send count %d", mqtt_message_count);
	display->drawString(0 + x, 36 + y, number_buffer);

	sprintf(number_buffer, "MQTT reconnect count %d", mqtt_disconnect_count);
	display->drawString(0 + x, 48 + y, number_buffer);
}
